...
public class TablePrinter {
     ...
     */
    public TablePrinter(ArrayList<String> field) {
        cols = new Col[field.size()];
        for(int i = 0; i < cols.length; i++) {
            cols[i] = new Col();
            cols[i].name = field.get(i);
    ...
    public void addRow(ArrayList<String> values) {
        ...
        for(int i = 0; i < values.size(); i++) {
            if(values.get(i).length() > cols[i].maxWidth) {
                cols[i].maxWidth = values.get(i).length();
            }
        }
    }
